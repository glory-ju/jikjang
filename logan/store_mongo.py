...
class NaverNewsCrawler:
    ...
    def conn_mongodb(config_t):
        ...
        return db_conn.set_mongodb(hostname, port, username, password, db_name)
        ...
        for subcategory, subcategory_name in zip(subcategory_value_list, subcategory_key_list):
            ...
            for page_url in made_urls:
                ...
                for id in range(len(url_list)):
                    ...
                    if soup_1.find('h3', attrs={'id': 'articleTitle'}).get_text():
                        title = soup_1.find('h3', attrs={'id': 'articleTitle'}).get_text()
                    elif soup_1.find('h3', attrs={'id': 'articleTitle'}).get_text() == None:
                        title = ''
                    else:
                    ...
                    if content != '' and key != '':
                        try:
                            db_conn.mongo_insert({'_id':key, 'sid':sid1, 'sid2':sid2, 'oid':oid, 'aid':aid,
