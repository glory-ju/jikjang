...
def find_news_totalpage(url, headers):
    try:
        totlapage_url = url
...
def find_news_article_url(page_url, headers):
    url_list = []
...
def find_news_content_elements(url_list, headers):
...
if __name__ == '__main__':
    ...
    while days <= 0:
        ...
        totalpage = find_news_totalpage(url + '&page=10000', headers)
        made_urls = [url + '&page=' + str(page) for page in range(1, totalpage + 1)]

        for page_url in made_urls:
            print(page_url)
            url_list = find_news_article_url(page_url, headers)

            for idx in range(len(url_list)):
                elements = find_news_content_elements(url_list, headers)
                news_info_list.append(elements)
            df = pd.DataFrame(news_info_list, columns=['today', 'sid1', 'sid2', 'oid', 'aid', 'time', 'media', 'writer', 'title', 'content', 'url'])
            df.to_csv('test.csv', index=False, encoding='utf-8')

            if page_url.split('=')[-1] == str(totalpage): days += 1